...
//! @todo Height,temperature should be a quantity_point, not a (relative?) quantity? How to deduce filter types? The multiply operator does not make sense for a quantity_point. Deducing the types is not quite correct?
using height = mp_units::quantity<mp_units::isq::height[m]>;
using temperature = mp_units::quantity<mp_units::isq::Celsius_temperature[deg_C]>;
} // namespace fcarouge
