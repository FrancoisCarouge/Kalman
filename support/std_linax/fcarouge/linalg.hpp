...
struct transposes<typed_matrix<Matrix, RowIndexes, ColumnIndexes>> {
  ...
  operator()(const typed_matrix<Matrix, RowIndexes, ColumnIndexes> &value)
      -> typed_matrix<evaluate<transpose<Matrix>>, ColumnIndexes, RowIndexes>;
  //   {
  //     return typed_matrix<evaluate<transpose<Matrix>>, ColumnIndexes,
  //     RowIndexes>{
  //         t(value.data())};
  //   }
};
...
template <typename Type, typename RowIndexes, typename ColumnIndexes>
std::array<Type,
           std::tuple_size_v<RowIndexes> * std::tuple_size_v<ColumnIndexes>>
    a{};
...
template <typename Type, typename RowIndexes, typename ColumnIndexes>
std::mdspan s{a<Type, RowIndexes, ColumnIndexes>.data(),
              e<RowIndexes, ColumnIndexes>};

// Don't provide a typed one / only a non-typed one.
template <typename Matrix, typename RowIndexes, typename ColumnIndexes>
inline typed_matrix<decltype(one<Matrix>), RowIndexes, ColumnIndexes>
    one<typed_matrix<Matrix, RowIndexes, ColumnIndexes>>{
        // TODO?
        s<double, RowIndexes, ColumnIndexes>};
...
//! @brief Scalar type indexed-based matrix with `std::mdspan` implementations.
template <typename Type = double, std::size_t Row = 1, std::size_t Column = 1>
using matrix =
    typed_matrix<std::mdspan<Type, std::extents<std::size_t, Row, Column>>,
...
template <typename Type = double, std::size_t R1 = 1, std::size_t R2 = 1,
          std::size_t C = 1>
constexpr auto
operator/(const matrix<Type, R1, C> &lhs,
          const matrix<Type, R2, C> &rhs) -> matrix<Type, R1, R2>;
