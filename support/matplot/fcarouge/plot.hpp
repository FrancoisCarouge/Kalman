...
//! @todo Support fixed size history buffer to avoid unbounded memory growth by
//! declaration?
//! @todo Support filepath?
...
template <typename Filter> plotter<Filter>::~plotter() {
  // Is there a Matplot++ type for serie already?
  using serie = std::vector<double>;

  auto plot2{[this](const std::string &characteristic_x,
                    const std::string &characteristic_y, const auto &serie_x,
                    const auto &serie_y) {
    std::string title{
  ...
  if constexpr (fcarouge::kalman_internal::has_output_uncertainty_method<
                    Filter>) {
  ...
  if constexpr (fcarouge::kalman_internal::has_state_transition<Filter>) {
  ...
  if constexpr (fcarouge::kalman_internal::has_output_model<Filter>) {
  ...
  if constexpr (fcarouge::kalman_internal::has_input_control<Filter>) {
  ...
  if constexpr (fcarouge::kalman_internal::has_gain<Filter>) {
    series["Gain K"] =
        history |
...
struct plotter {
  ...
  static plotter operator()(const std::string &plot_name) {
    return plotter{plot_name};
  }
};

template <typename Filter>
auto operator|(Filter &&filter, const plotter &decorator) {
