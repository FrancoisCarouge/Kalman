...
[[maybe_unused]] auto benchmark{[] {
  ...
  const auto predict_name{std::format("predict_{}x{}", std::size_t{StateSize}, std::size_t{InputSize})}; 

  bench.run(predict_name, [&]() { filter.predict(vector<InputSize>{}); });

  return 0;
}};

[[maybe_unused]] auto run{benchmark<${STATE_SIZE}, 1, ${IO_SIZE}>()};
} // namespace
} // namespace fcarouge::benchmark
