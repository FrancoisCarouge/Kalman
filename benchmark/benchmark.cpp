...
[[maybe_unused]] auto benchmark{[] {
  ...
  fcarouge::kalman_internal::for_constexpr<1, 5, 1>([&](auto state_size) {
    fcarouge::kalman_internal::for_constexpr<1, 5, 1>([&](auto output_size) {
      fcarouge::kalman_internal::for_constexpr<1, 5, 1>([&](auto input_size) {
        ...
        // const auto predict_name{std::format(
        //     "predict_{}x{}", std::size_t{state_size},
        //     std::size_t{input_size})};
        // bench.run(predict_name,
        //           [&]() { filter.predict(vector<input_size>{}); });

        const auto update_name{std::format(
            "update_{}x{}", std::size_t{state_size}, std::size_t{output_size})};
        bench.run(update_name, [&]() { filter.update(vector<output_size>{}); });
      });
    });
  });
...
[[maybe_unused]] auto run{benchmark()};
} // namespace
} // namespace fcarouge::benchmark
