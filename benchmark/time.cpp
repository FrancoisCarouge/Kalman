...
const std::string csv{std::format(
    R"(# State Size, Output Size, Input Size, Function, Representation, Elapsed Time (s)
{{{{#result}}}}{:03d}, {:03d}, {:03d}, {{{{name}}}}, ${REPRESENTATION}, {{{{minimum(elapsed)}}}}{{{{/result}}}}
)",
    state_size, output_size, input_size)};
...
void bench() {
  ...
  std::ofstream results{"${CMAKE_SOURCE_DIR}/benchmark/time.csv",
                        std::ios::app};
