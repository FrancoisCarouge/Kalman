...
[[maybe_unused]] auto sample{[] {
  ...
  std::cout << std::format("F = {}", filter.f()) << std::endl;
  std::cout << std::format("H = {}", filter.h()) << std::endl;
  std::cout << std::format("K = {}", filter.k()) << std::endl;
  std::cout << std::format("P = {}", filter.p()) << std::endl;
  std::cout << std::format("Q = {}", filter.q()) << std::endl;
  std::cout << std::format("R = {}", filter.r()) << std::endl;
  std::cout << std::format("S = {}", filter.s()) << std::endl;
  // Do we want to specialize format?
  std::cout << std::format("X = {}", filter.x().quantity_from_zero())
            << std::endl;
  std::cout << std::format("Y = {}", filter.y()) << std::endl;
  std::cout << std::format("Z = {}", filter.z().quantity_from_zero())
            << std::endl;
  ...
  //   double f = 5;
  //   auto result{x * f};
  //   std::cout << std::format("result = {}", result.quantity_from_zero())
  //             << std::endl;
  ...
  /*
          assert(temperature{10 * deg_C} == filter.x() &&
                 "Since our model has constant dynamics, the predicted estimate
    is " "equal to the current estimate: x^1,0 = 10°C."); assert(10'000.000'1 *
      deg_C2 == filter.p() && "The extrapolated estimate uncertainty (variance):
      p1,0 = p0,0 + q = " "10'000 + 0.000'1 = 10'000.000'1.");
*/
  ...
  /*
      ...
      // The estimate uncertainty quickly goes down, after 10 measurements:
          assert(abs(1 - filter.p() / 0.001'3) < 0.05 &&
                 ...
                 "The estimate uncertainty is 0.0013, i.e. the estimate error
         standard " "deviation is: 0.036°C."); assert(abs(1 - filter.x() /
         temperature{49.988
          * deg_C}) < 0.001 && "The state estimates expected at 0.1% accuracy."
      "The filter estimates the liquid temperature at 49.988°C."); assert(abs(1
     - filter.k() / 0.126'5) < 0.001 && "The gain expected at 0.1% accuracy.");

          // So we can say that the liquid temperature estimate is: 49.988 ±
          0.036°C.
              // In this example we've measured a liquid temperature using the
              // one-dimensional Kalman filter. Although the system dynamics
      include
              // a random process noise, the Kalman filter can provide good
              // estimation.
              */
  return 0;
}()};
