...
[[maybe_unused]] auto sample{[] {
  ...
  kalman filter{
      ...
      state{0., 0.},
      // The filter estimates the altitude as a double [m].
      output<altitude>,
      // The filter receives in the accelerometer as a double [m.s^-2].
      input<acceleration>,
      // Since our initial state vector is a guess, we will set a very high
      // estimate uncertainty. The high estimate uncertainty results in high
      // Kalman gain, giving a high weight to the measurement.
      estimate_uncertainty{{500., 0.}, {0., 500.}},
      // The filter prediction uses a delta time [ms].
