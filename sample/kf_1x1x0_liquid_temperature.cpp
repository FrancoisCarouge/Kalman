...
[[maybe_unused]] auto sample{[] {
  ...
  kalman filter{
      ...
      // predict the next state (which is the first state). We don't know what
      // the temperature of the liquid is, and our guess is: estimated state x =
      // 10°C.
      state{10.},
      // Our guess is very imprecise, so we set our initialization estimate
      // error σ to 100. The estimate uncertainty of the initialization is the
      // error variance σ^2: p0,0 = 100^2 = 10,000. This variance is very high.
      // If we initialize with a more meaningful value, we will get faster
      ...
      // σ^2 would be 0.01, thus the measurement, output uncertainty is: r1 =
