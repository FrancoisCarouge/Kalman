...
constexpr auto fcarouge::operator/(const Numerator &lhs, const Denominator &rhs)
  ...
  // return rhs.transpose().llt().solve(lhs.transpose()).transpose();
  auto trhs{transpose(rhs)};
  auto decomp{llt(trhs)};
  auto tlhs{transpose(lhs)};
  auto sol{solve(decomp, tlhs)};
  auto tsol{transpose(sol)};
  return tsol;
}
...
[[maybe_unused]] auto test{[] {
  ...
  std::cout << q(3, 1) << std::endl;
  std::cout << q(4, 0) << std::endl;
  std::cout << q(4, 1) << std::endl;
  std::cout << q(5, 0) << std::endl;
